...
const UserData = [
  ...
  amount: 30000,
  duration: 12,
  Start_date: new Date("2018-08-18"),
  category: "Internally Funded",
  progress: [
  { direction: 0, resource_utilization: 0, satisfaction: 0, comments: "" }
  ]
  },
  {
  ...
  {
  project_name: "Smart Accident Detection and Rescue System (SADRS)",
  Students: ["Nikitha Srinivasan", "Muthumari Sudharson"],
  guides: ["Dr. I. Joe Louis Paul"],
  amount: 10000,
  duration: 9,
  Start_date: new Date("2018-08-18"),
  ...
  {
  project_name: "Digital E-Spotter",
  Students: ["Shwetha S. Kumar", "S. Surya", "N. Swathi"],
  guides: ["Dr. I. Joe Louis Paul", "Ms. R. Swathika"],
  amount: 14000,
  duration: 9,
  Start_date: new Date("2018-08-18"),
  category: "Internally Funded",
  ...
  {
  project_name: "Smart farming with cloud support",
  Students: ["Anirudh V. Ram", "Sri Raksha T. Reddy"],
  guides: ["Dr. S. Karthika", "Ms. R. Swathika"],
  amount: 18000,
  duration: 11,
  Start_date: new Date("2018-08-18"),
  category: "Internally Funded",
  ...
  amount: 18000,
  duration: 18,
  Start_date: new Date("2020-01-31"),
  category: "Engineering",
  progress: [{ direction: 0, resource_utilization: 0, satisfaction: 0, comments: "" }]
  },
  {
  ...
  amount: 22000,
  duration: 12,
  Start_date: new Date("2022-05-10"),
  category: "Engineering",
  progress: [{ direction: 0, resource_utilization: 0, satisfaction: 0, comments: "" }]
  },
  {
...
mongoose.connect(
  "mongodb+srv://keerthu:chellam2004@cluster0.cqttcna.mongodb.net/projects?retryWrites=true&w=majority&appName=Cluster0",
  {
...
app.post("/api/projects", async (req, res) => {
  try {
    for(const d of UserData){
      ...
      teamleaders = ["mathavaroopan2210503@ssn.edu.in", "meduri2210112@ssn.edu.in"]
      team = Math.floor(Math.random() * 2);
      console.log(Students);
      team_leader = teamleaders[team];
      research_papers = []
      patents = []
      ppts = []
      ...
      // Don't send the response here
      console.log(newProject);
      console.log("Insert successful");
    }
    
    // Send the response after the loop completes
    res.status(201).json({ message: "Projects created successfully" });
...
app.post("/api/progress", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "Progress inserted successfully", project });
  } catch (error) {
...
app.post("/file-upload", async (req, res) => {
  try {
    ...
    bb.on("file", (fieldname, file, filename, encoding, mimetype) => {
      ...
      file.pipe(fs.createWriteStream(filePath + extension));
      ...
      file.on("close", async () => {
        ...
        fileData.path = "uploads/" + tmpFilename + extension;
        // fileData.userId = user.id

        const fileStat = fs.statSync("./uploads/" + tmpFilename + extension);
